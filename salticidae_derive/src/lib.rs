...
fn variants_to_serializers(variants: syn::punctuated::Iter<'_, syn::Variant>, name: &syn::Ident) -> TokenStream2 {
    variants
        .map(|varient| -> TokenStream2 {
            variant_to_serializers(varient, name)
        })
...
fn type_to_string(ty: &syn::Type) -> String {
    match ty {
        syn::Type::Path(syn::TypePath {
            ...
            Some(syn::PathSegment { ident, .. }) => ident.to_string(),
            _ => "".to_string(),
        },
        _ => "".to_string(),
    }
